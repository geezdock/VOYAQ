...
export function HubOverview({ data, squad }: HubOverviewProps) {
  const countdown = squad.lockedDates ? getCountdown(squad.lockedDates.start) : null;
  const totalBudget = data.budgetInsights.reduce((sum, b) => sum + b.estimatedCost, 0);
  const budgetRatio = squad.lockedBudget ? totalBudget / squad.lockedBudget : 0;
    ...
    <div className="space-y-6">
      ...
      >
        <div className="p-5 sm:p-6 space-y-4">
          <p className="font-heading text-sm text-ink-muted leading-relaxed">{data.overview.description}</p>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {data.overview.quickFacts.map((fact) => (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        >
          ...
          <p className="font-display text-xl font-extrabold text-ink">{data.weather.current.temp}°C</p>
          <p className="font-heading text-xs text-ink-muted">{data.weather.current.condition}</p>
        </motion.div>
        ...
        >
          ...
          <span className={`inline-block font-mono text-[10px] px-2 py-0.5 rounded-bruted border ${budgetBg}`}>
            {formatRupee(squad.lockedBudget ?? totalBudget)}/person
          </span>
        </motion.div>
        ...
        >
          ...
          <p className="font-display text-xl font-extrabold text-ink">{data.events.length}</p>
          <p className="font-heading text-xs text-ink-muted">
            {data.events.length === 0 ? "No upcoming" : data.events.length === 1 ? "Event this season" : "Events this season"}
          </p>
        </motion.div>
        ...
        >
          ...
          <p className="font-display text-xl font-extrabold text-ink">{data.advisories.length}</p>
          <p className="font-heading text-xs text-ink-muted">
            {data.advisories.length === 0 ? "All clear" : "Active notices"}
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          className="border-2 border-ink/10 rounded-[12px] p-4 bg-surface-alt/50 space-y-2"
        >
          <UtensilsCrossed className="w-5 h-5 text-[#D4836A]" />
          <p className="font-mono text-[10px] text-ink-muted uppercase tracking-wider">Top Pick</p>
          <p className="font-heading text-sm font-bold text-ink leading-tight">{data.food[0]?.name ?? "—"}</p>
          <p className="font-heading text-xs text-ink-muted truncate">{data.food[0]?.category ?? ""}</p>
        </motion.div>
      </div>
      ...
      {data.aiSuggestions.length > 0 && (
        <motion.div
          ...
          className="border-2 border-accent/30 rounded-[12px] p-4 bg-accent/5 space-y-2"
        >
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-accent" />
            <p className="font-heading text-sm font-bold text-ink">AI Suggestion</p>
          </div>
          <p className="font-heading text-sm text-ink-muted leading-relaxed">
            {data.aiSuggestions[0].tip}
          </p>
        </motion.div>
      )}
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div>
            ...
            <p className="font-heading text-xs font-semibold text-ink">{data.overview.bestTimeToVisit}</p>
          </div>
          <div>
            <p className="font-mono text-[10px] text-ink-muted uppercase">Language</p>
            <p className="font-heading text-xs font-semibold text-ink">{data.overview.language}</p>
          </div>
          <div>
            <p className="font-mono text-[10px] text-ink-muted uppercase">Currency</p>
            <p className="font-heading text-xs font-semibold text-ink">{data.overview.currency}</p>
          </div>
          <div>
            <p className="font-mono text-[10px] text-ink-muted uppercase">Time zone</p>
            <p className="font-heading text-xs font-semibold text-ink">{data.overview.timeZone}</p>
          </div>
        </div>
      </motion.div>
    </div>
