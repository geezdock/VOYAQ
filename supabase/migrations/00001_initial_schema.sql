...
create index idx_destinations_squad on destinations(squad_id);
...
create index idx_destination_votes_squad on destination_votes(squad_id);
...
create index idx_budget_prefs_squad on budget_preferences(squad_id);
...
create index idx_date_proposals_squad on date_proposals(squad_id);
...
create index idx_dpv_proposal on date_proposal_votes(date_proposal_id);
...
create index idx_polls_squad on polls(squad_id);
...
create index idx_parental_consents_profile on parental_consents(profile_id);
...
create policy "Squad creator can update"
  on squads for update
  using (created_by = auth.uid());

create policy "Squad creator can delete"
...
create policy "Squad creator can manage members"
...
create policy "Destinations are viewable by squad members"
  ...
  using (
    exists (
      select 1 from squad_members
      where squad_members.squad_id = destinations.squad_id
      and squad_members.profile_id = auth.uid()
    )
  );
...
create policy "Votes are viewable by squad members"
  ...
  using (
    exists (
      select 1 from squad_members
      where squad_members.squad_id = destination_votes.squad_id
      and squad_members.profile_id = auth.uid()
    )
  );
...
create policy "Budget prefs are viewable by squad members"
  ...
  using (
    exists (
      select 1 from squad_members
      where squad_members.squad_id = budget_preferences.squad_id
      and squad_members.profile_id = auth.uid()
    )
  );
...
create policy "Users can update own budget"
  on budget_preferences for update
  using (member_id = auth.uid());

create policy "Users can delete own budget"
  on budget_preferences for delete
  using (member_id = auth.uid());
...
create policy "Date proposals are viewable by squad members"
  ...
  using (
    exists (
      select 1 from squad_members
      where squad_members.squad_id = date_proposals.squad_id
      and squad_members.profile_id = auth.uid()
    )
  );
...
create policy "Proposer can delete own proposal"
  on date_proposals for delete
...
create policy "Date votes are viewable by squad members"
  ...
  using (
    exists (
      ...
      join squad_members sm on sm.squad_id = dp.squad_id
      where dp.id = date_proposal_votes.date_proposal_id
      and sm.profile_id = auth.uid()
    )
  );

create policy "Users can manage own date votes"
  on date_proposal_votes for insert
  with check (member_id = auth.uid());

create policy "Users can delete own date votes"
  on date_proposal_votes for delete
  using (member_id = auth.uid());
...
create policy "Poll votes are viewable by squad members"
  ...
  using (
    exists (
      ...
      join polls on polls.id = po.poll_id
      join squad_members on squad_members.squad_id = polls.squad_id
